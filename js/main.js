...
function isLevelConfigValid(level) {
  if (!level || level.columns <= 0 || level.rows <= 0 || level.timeLimit <= 0 || !level.drone) {
    return false;
  }
  ...
  const isWithinBounds = (point) => (
    ...
    && point.x >= 0
    && point.y >= 0
    && point.x < level.columns
    && point.y < level.rows
  );

  const isDroneAxisValid = level.drone.axis === 'x' || level.drone.axis === 'y';
  const isDroneRangeValid = Number.isFinite(level.drone.min)
    && Number.isFinite(level.drone.max)
    && level.drone.min <= level.drone.max;
  const isDroneSpeedValid = Number.isFinite(level.drone.speed) && level.drone.speed > 0;
  const isDroneStartValid = isWithinBounds(level.drone.start);

  if (!isWithinBounds(level.start) || !isWithinBounds(level.exit) || !isWithinBounds(level.core)) {
    return false;
  }

  const hasExpectedCoreTile = level.initialLayout[level.core.y]?.[level.core.x] === TILE_TYPES.ITEM;
  const hasExpectedExitTile = level.initialLayout[level.exit.y]?.[level.exit.x] === TILE_TYPES.GOAL;
...
function refreshUi(feedbackMessage) {
  updateMissionUi();
...
function collectCore() {
  ...
  refreshUi(MESSAGES.coreCollected);
}
...
function energizeDoor() {
  ...
  refreshUi(MESSAGES.doorEnergized);
}
